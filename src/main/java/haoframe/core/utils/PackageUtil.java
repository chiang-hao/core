...
public class PackageUtil {
	...
	public static Set<Class<?>> findAnnotationClass(String scanPackages,Class<? extends Annotation> annotation){
		Set<String> clazzSet = findPackageClass(scanPackages);
		Set<Class<?>> classes = new HashSet<Class<?>>();
		for (String clazz : clazzSet) {
			try {
				Class<?> clz = backAnnotationClass(clazz, annotation);
				if(clz != null) {
					classes.add(clz);
				}
	...
	public static Set<String> findPackageClass(String scanPackages) {
		...
		for (String basePackage : packages) {
			...
			try {
				...
				System.out.println("获取包下面的类信息失败,package:" + basePackage);
			}
	...
	private static String loadClassName(MetadataReaderFactory metadataReaderFactory, Resource resource)
		...
		try {
			...
			System.out.println("根据resource获取类名称失败");
		}
		return null;
	}
	...
	public static Class<?> backAnnotationClass(String fullClassName, Class<? extends Annotation> anno)
			throws ClassNotFoundException {
		Class<?> clz = Class.forName(fullClassName);
	...
	public static void main(String[] args) {
		// String packages =
		// "com.a,com.ab,com.c,com.as.t,com.as,com.as.ta,com.at.ja,com.at.jc,com.at.";
		String packages = "hao.webapp.model";
		System.out.println("检测前的package: " + packages);
		System.out.println("检测前的package: " + findPackageClass(packages));
